...
# On-screen keyboard acceptance for the private disposable VM lab.

omarchy_host_test() {
  local plugin_dir
  local fixture_dir
  local remote_plugin_dir="${ONSCREEN_KEYBOARD_PLUGIN_GUEST_DIR:-/tmp/omarchy-onscreen-keyboard}"
  local remote_fixture_dir="/tmp/omarchy-onscreen-keyboard-fixtures"
  local probe_address
  local target_address
  local expected_version
  plugin_dir=${ONSCREEN_KEYBOARD_PLUGIN_SOURCE:?Set ONSCREEN_KEYBOARD_PLUGIN_SOURCE to the on-screen keyboard plugin checkout}
  fixture_dir="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../fixtures/onscreen-keyboard" && pwd)"
  [[ -d $plugin_dir/.git && -f $plugin_dir/manifest.json ]] || {
    printf 'Invalid ONSCREEN_KEYBOARD_PLUGIN_SOURCE: %s\n' "$plugin_dir" >&2
    return 1
  }
  expected_version=$(jq -er '.version' "$plugin_dir/manifest.json") || return 1

  log "Staging Omarchy On-Screen Keyboard in the disposable guest"
  ...
  ssh_guest "cc -O2 -Wall -Wextra \
    ...
    -o /tmp/omarchy-onscreen-keyboard-gtk-target \
  ...
  ssh_session "rm -rf \"\$HOME/.config/omarchy/plugins/dev.omarchy.onscreen-keyboard\"; \
    omarchy-plugin-add ${remote_plugin_dir} --enable --yes" || return 1
  wait_for_guest_state "on-screen keyboard plugin is installed and enabled" 20 ssh_session \
    "test -f \"\$HOME/.config/omarchy/plugins/dev.omarchy.onscreen-keyboard/manifest.json\" && \
  ...
  ssh_session "rm -f /tmp/onscreen-keyboard-input.hex /tmp/onscreen-keyboard-target.log; \
    omarchy-launch-tui --app-id=onscreen-keyboard-test \
      python ${remote_fixture_dir}/input-target.py /tmp/onscreen-keyboard-input.hex \
      >/tmp/onscreen-keyboard-target.log 2>&1 &" || return 1
  wait_for_guest_state "controlled input target has keyboard focus" 20 ssh_session \
    "hyprctl -j activewindow | jq -e '.class == \"onscreen-keyboard-test\"'" || return 1
  target_address=$(ssh_session "hyprctl -j activewindow | jq -r '.address'") || return 1
  capture_console "success-onscreen-keyboard-01-hidden-1280x800"
  ...
  capture_console "success-onscreen-keyboard-02-visible-1280x800"
  ...
  long_press_at() {
    ...
    wait_for_guest_state "long press opens the developer alternative" 5 ssh_session \
      "omarchy-shell onscreen-keyboard status | jq -e '.alternatesVisible == true'" || return 1
  }
  ...
  capture_console "success-onscreen-keyboard-02b-bar-click-1280x800"

  log "Proving compositor shortcuts from the touch keyboard"
  ssh_session "omarchy-launch-tui --app-id=tablet-shortcut-probe sleep 120 >/tmp/onscreen-keyboard-shortcut-probe.log 2>&1 &" || return 1
  ...
  wait_for_guest_state "Alt+Tab cycles away from the shortcut probe" 10 ssh_session \
    "test \"\$(hyprctl -j activewindow | jq -r '.address')\" != '$probe_address' && \
     omarchy-shell onscreen-keyboard status | jq -e '.busy == false and ([.modifiers[]] | any) == false'" || {
      ssh_session "printf 'active after Alt+Tab: '; hyprctl -j activewindow | jq -c '{address,class,title}'; \
        printf 'keyboard status: '; omarchy-shell onscreen-keyboard status"
      return 1
    }
  ...
  wait_for_guest_state "touch Super+Space opens the Omarchy menu without wtype" 10 ssh_session \
    "hyprctl -j layers | jq -e '[.. | objects | select(.namespace? == \"omarchy-menu\")] | length >= 1' && \
     omarchy-shell onscreen-keyboard status | jq -e '.busy == false and ([.modifiers[]] | any) == false'" || return 1
  ...
  wait_for_guest_state "three-modifier shortcut state is visibly latched" 5 ssh_session \
    "omarchy-shell onscreen-keyboard status | jq -e '.modifiers.ctrl and .modifiers.shift and .modifiers.super and (.modifiers.alt | not)'" || return 1
  ...
  wait_for_guest_state "Super+Shift+Ctrl+Space opens the Omarchy theme menu" 10 ssh_session \
    "hyprctl -j layers | jq -e '[.. | objects | select(.namespace? == \"omarchy-image-selector\")] | length >= 1' && \
     omarchy-shell onscreen-keyboard status | jq -e '.busy == false and ([.modifiers[]] | any) == false'" || {
      ssh_session "printf 'keyboard status after theme chord: '; omarchy-shell onscreen-keyboard status; \
        printf 'theme layers: '; hyprctl -j layers | jq -c '[.. | objects | select(.namespace? == \"omarchy-image-selector\")]'"
      return 1
    }
  capture_console "success-onscreen-keyboard-02d-touch-shortcut-theme-menu"
  ...
  wait_for_guest_state "Shift state is visible" 5 ssh_session \
    "omarchy-shell onscreen-keyboard status | jq -e '.shift == true'" || return 1
  capture_console "success-onscreen-keyboard-03-shift-1280x800"
  ...
  wait_for_guest_state "Ctrl is visibly latched" 5 ssh_session \
    "omarchy-shell onscreen-keyboard status | jq -e '.modifiers.ctrl == true'" || return 1
  ...
  wait_for_guest_state "Alt is visibly latched" 5 ssh_session \
    "omarchy-shell onscreen-keyboard status | jq -e '.modifiers.alt == true'" || return 1
  ...
  wait_for_guest_state "Super is visibly latched without firing a global shortcut" 5 ssh_session \
    "omarchy-shell onscreen-keyboard status | jq -e '.modifiers.super == true'" || return 1
  capture_console "success-onscreen-keyboard-03b-super-latched-1280x800"
  ...
  wait_for_guest_state "controlled target completes the input sequence" 10 ssh_session \
    "test -f /tmp/onscreen-keyboard-input.hex" || {
      ssh_session "printf 'active window: '; hyprctl -j activewindow | jq -c '{address,class,title}'; \
        printf 'keyboard status: '; omarchy-shell onscreen-keyboard status; \
        printf 'target log: '; cat /tmp/onscreen-keyboard-target.log 2>/dev/null || true"
      return 1
    }
  ssh_session "printf 'received input hex: '; cat /tmp/onscreen-keyboard-input.hex" || return 1
  ssh_session "grep -qx '614221201b5b447f1b5b43091b031b780d' /tmp/onscreen-keyboard-input.hex" || return 1
  printf 'ok - terminal control receives exact keys, named Space, and Ctrl/Alt chords\n'
  ssh_session "test \"\$(hyprctl -j activewindow | jq -r '.address')\" = '$target_address'" || return 1
  ssh_session "omarchy-shell onscreen-keyboard status | jq -e '.shift == false and ([.modifiers[]] | any) == false and .backend == \"ready\" and .busy == false'" || return 1
  ...
  ssh_session "hyprctl eval 'hl.dispatch(hl.dsp.window.close({ window = \"address:$target_address\" }))' >/dev/null; \
    rm -f /tmp/onscreen-keyboard-chromium-result /tmp/onscreen-keyboard-chromium-server.pid; \
    python ${remote_fixture_dir}/chromium-input-target.py /tmp/onscreen-keyboard-chromium-result \
      >/tmp/onscreen-keyboard-chromium-server.log 2>&1 & echo \$! >/tmp/onscreen-keyboard-chromium-server.pid; \
    chromium --ozone-platform=wayland --user-data-dir=/tmp/onscreen-keyboard-chromium-profile \
      --no-first-run --no-default-browser-check --disable-sync --disable-extensions \
      --app=http://127.0.0.1:18473/ >/tmp/onscreen-keyboard-chromium.log 2>&1 &" || return 1
  ...
  wait_for_guest_state "Chromium receives Space between printable keys" 10 ssh_session \
    "test \"\$(cat /tmp/onscreen-keyboard-chromium-result 2>/dev/null)\" = 'a b'" || return 1
  long_press_at 1280 800 915 659 || return 1 # Semicolon opens Colon
  ssh_session "test \"\$(hyprctl -j activewindow | jq -r '.address')\" = '$target_address'" || return 1
  capture_console "success-onscreen-keyboard-04a-chromium-long-press"
  ...
  wait_for_guest_state "Chromium receives the exact developer alternative" 10 ssh_session \
    "test \"\$(cat /tmp/onscreen-keyboard-chromium-result 2>/dev/null)\" = 'a b:' && \
     omarchy-shell onscreen-keyboard status | jq -e '.alternatesVisible == false'" || return 1
  tap_at 1280 800 150 767 || return 1      # Ctrl
  ...
  wait_for_guest_state "Chromium receives the exact rapid/chord sequence" 15 ssh_session \
    "test \"\$(od -An -tx1 /tmp/onscreen-keyboard-chromium-result | tr -d ' \n')\" = '427171200a'" || return 1
  ssh_session "test \"\$(hyprctl -j activewindow | jq -r '.address')\" = '$target_address' && \
    omarchy-shell onscreen-keyboard status | jq -e '.busy == false and ([.modifiers[]] | any) == false'" || return 1
  capture_console "success-onscreen-keyboard-04-chromium-input-1280x800"

  log "Proving named Space, Ctrl/Shift chords, navigation and Enter in GTK"
  ssh_session "hyprctl eval 'hl.dispatch(hl.dsp.window.close({ window = \"address:$target_address\" }))' >/dev/null; \
    kill \"\$(cat /tmp/onscreen-keyboard-chromium-server.pid)\"; \
    rm -f /tmp/onscreen-keyboard-gtk-result /tmp/onscreen-keyboard-gtk-activated; \
    /tmp/omarchy-onscreen-keyboard-gtk-target /tmp/onscreen-keyboard-gtk-result /tmp/onscreen-keyboard-gtk-activated \
      >/tmp/onscreen-keyboard-gtk.log 2>&1 &" || return 1
  ...
  wait_for_guest_state "GTK receives Space between printable keys" 10 ssh_session \
    "test \"\$(cat /tmp/onscreen-keyboard-gtk-result 2>/dev/null)\" = 'a b'" || return 1
  ...
  wait_for_guest_state "GTK receives the exact developer alternative" 10 ssh_session \
    "test \"\$(cat /tmp/onscreen-keyboard-gtk-result 2>/dev/null)\" = 'a b?' && \
     omarchy-shell onscreen-keyboard status | jq -e '.alternatesVisible == false'" || return 1
  tap_at 1280 800 150 767 || return 1      # Ctrl
  tap_at 1280 800 123 659 || return 1      # Ctrl+A
  tap_at 1280 800 125 710 || return 1      # Shift
  ...
  wait_for_guest_state "GTK receives the exact rapid/chord sequence and activation" 15 ssh_session \
    "test \"\$(cat /tmp/onscreen-keyboard-gtk-result 2>/dev/null)\" = 'Cddd' && \
     grep -qx activated /tmp/onscreen-keyboard-gtk-activated" || return 1
  ssh_session "test \"\$(hyprctl -j activewindow | jq -r '.address')\" = '$target_address'" || return 1
  capture_console "success-onscreen-keyboard-05-gtk-input-1280x800"

  log "Proving backend failure clears one-shot state and pending actions"
  ssh_session "touch /tmp/omarchy-onscreen-keyboard-wtype-fail" || return 1
  ...
  wait_for_guest_state "backend failure is contained without a stuck modifier" 10 ssh_session \
    "omarchy-shell onscreen-keyboard status | jq -e '.backend == \"backend-error\" and .busy == false and ([.modifiers[]] | any) == false' && \
     test \"\$(cat /tmp/onscreen-keyboard-gtk-result)\" = 'Cddd' && ! pgrep -u \"\$(id -u)\" -x wtype >/dev/null" || return 1
  prove_physical_super_space "backend failure" || return 1
  ssh_session "rm -f /tmp/omarchy-onscreen-keyboard-wtype-fail" || return 1

  log "Proving hide terminates a live held-modifier backend and releases Ctrl"
  ssh_session "touch /tmp/omarchy-onscreen-keyboard-wtype-hold" || return 1
  tap_at 1280 800 150 767 0.01 || return 1 # Ctrl
  ...
  wait_for_guest_state "held modifier backend is active" 10 ssh_session \
    "omarchy-shell onscreen-keyboard status | jq -e '.busy == true' && pgrep -u \"\$(id -u)\" -x wtype >/dev/null" || return 1
  ssh_session "omarchy-shell onscreen-keyboard hide | grep -qx ok" || return 1
  wait_for_guest_state "hide cancels the backend and clears all modifier state" 10 ssh_session \
    "omarchy-shell onscreen-keyboard status | jq -e '.visible == false and .busy == false and ([.modifiers[]] | any) == false' && \
     ! pgrep -u \"\$(id -u)\" -x wtype >/dev/null" || return 1
  ssh_session "rm -f /tmp/omarchy-onscreen-keyboard-wtype-hold; /usr/bin/wtype x" || return 1
  wait_for_guest_state "post-hide input proves Ctrl is not stuck" 10 ssh_session \
    "test \"\$(cat /tmp/onscreen-keyboard-gtk-result)\" = 'Cddxd'" || return 1
  ...
  capture_console "success-onscreen-keyboard-04-hidden-1920x1200"
  ssh_session "omarchy-shell onscreen-keyboard show | grep -qx ok" || return 1
  wait_for_guest_state "keyboard appears at reference viewport" 10 ssh_session \
    "hyprctl -j layers | jq -e '[.. | objects | select(.namespace? == \"omarchy-onscreen-keyboard\")] | length == 1'" || return 1
  capture_console "success-onscreen-keyboard-05-visible-1920x1200"
  tap_at 1920 1200 445 1112 || return 1
  wait_for_guest_state "Shift appears at reference viewport" 5 ssh_session \
    "omarchy-shell onscreen-keyboard status | jq -e '.shift == true'" || return 1
  capture_console "success-onscreen-keyboard-06-shift-1920x1200"
  tap_at 1920 1200 445 1112 || return 1
  ssh_session "printf '%s\n' '{\"switches\":[{\"name\":\"Asus WMI hotkeys\"}],\"keyboards\":[{\"name\":\"at-translated-set-2-keyboard\"},{\"name\":\"asustek-computer-inc.-n-key-device\"}]}' \
      > /tmp/omarchy-onscreen-keyboard-detector-devices.json; \
  ...
  wait_for_guest_state "scaled keyboard layer and client edges converge" 15 ssh_session \
    "client_bottom=\$(hyprctl -j activewindow | jq '.at[1] + .size[1]'); \
     ...
     delta=\$((client_bottom - keyboard_top)); \
     test \"\$delta\" -ge -2 && test \"\$delta\" -le 2" || return 1
  capture_console "success-onscreen-keyboard-06b-visible-1920x1200-scale1.6"
  ssh_session "omarchy-shell onscreen-keyboard hide >/dev/null" || return 1

  log "Proving disable, rescan, re-enable and removal cleanup"
  ssh_guest "git -C ${remote_plugin_dir} -c user.name='On-screen keyboard gate' -c user.email='onscreen-keyboard-gate.invalid' \
    commit --allow-empty -m 'Disposable update fixture' >/dev/null" || return 1
  ssh_session "omarchy plugin update dev.omarchy.onscreen-keyboard --yes" || return 1
  ...
  ssh_session "omarchy-plugin-disable dev.omarchy.onscreen-keyboard" || return 1
  wait_for_guest_state "disable unloads service, widget and keyboard layer" 15 ssh_session \
    "omarchy-plugin-list --json | jq -e 'any(.[]; .id == \"dev.omarchy.onscreen-keyboard\" and .enabled == false)' && \
     hyprctl -j layers | jq -e '[.. | objects | select(.namespace? == \"omarchy-onscreen-keyboard\")] | length == 0' && \
     ! omarchy-shell onscreen-keyboard status >/dev/null 2>&1" || return 1
  ...
  wait_for_guest_state "removal leaves no plugin config, process or layer residue" 20 ssh_session \
    "test ! -e \"\$HOME/.config/omarchy/plugins/dev.omarchy.onscreen-keyboard\" && \
     omarchy-plugin-list --json | jq -e 'all(.[]; .id != \"dev.omarchy.onscreen-keyboard\")' && \
     jq -e '[.. | objects | .id? // empty] | all(. != \"dev.omarchy.onscreen-keyboard\")' \"\$HOME/.config/omarchy/shell.json\" && \
     hyprctl -j layers | jq -e '[.. | objects | select(.namespace? == \"omarchy-onscreen-keyboard\")] | length == 0' && \
  ...
  ssh_session "rm -f /tmp/omarchy-onscreen-keyboard-detector-devices.json /tmp/omarchy-onscreen-keyboard-wtype-fail \
    /tmp/omarchy-onscreen-keyboard-wtype-hold" || return 1
  capture_console "success-onscreen-keyboard-07-removed"

  printf 'ok - on-screen keyboard lifecycle, pointer typing, focus and cleanup passed\n'
}
